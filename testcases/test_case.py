...
def test_send_constant_sms(test_login_success, env_config, phone):
    ...
    if login_url.__eq__("https://cloudsit.cm253.com/control/login"):
        ...
        logger.info(
            f"断言 页面实际短信内容:{content} 程序设置的短信断言内容:{assert_constant_template}\n 页面实际短信状态:{report} 程序设置的短信断言状态:{assert_report}")
    else:
        assert page.get_element(page.sendRecord).text == assert_constant_template
        logger.info(
            f"断言 页面实际短信内容:{page.get_element(page.sendRecord).text} 程序设置的短信断言内容:{assert_constant_template}\n ")
...
def test_send_variable_sms(test_login_success, env_config):
    ...
    if login_url.__eq__("https://cloudsit.cm253.com/control/login"):
        ...
        logger.info(
            f"断言 页面实际短信内容:{content} 程序设置的短信断言内容:{assert_variable_template}\n 页面实际短信状态:{report} 程序设置的短信断言状态:{assert_report}")
    else:
        assert page.get_element(page.sendRecord).text == assert_variable_template
        logger.info(
            f"断言 页面实际短信内容:{page.get_element(page.sendRecord).text} 程序设置的短信断言内容:{assert_variable_template}\n ")
